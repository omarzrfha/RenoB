...
// قم بقراءة البيانات من ملف Data.json إذا كان موجودًا
if (fs.existsSync(dataFilePath)) {
  const rawData = fs.readFileSync(dataFilePath);
  data = JSON.parse(rawData);
}

const allowedRoleIds = ['957442638590644280', '1119629846331543602', '1121528155044315227', '964733661930983504'];
const roleIdToRemove = '957442638607446085';
const maxRemovalsPerWeek = 2;
const removalCooldownPeriod = 7 * 24 * 60 * 60 * 1000; // 7 days in milliseconds
...
client.on('messageCreate', (message) => {
  if (message.content.startsWith(prefix + 'ro')) {
    ...
    if (hasAllowedRole) {
      if (!data[memberId]) {
        // إذا لم يكن لدينا سجل للعضو بعد، أو إذا كانت المدة منتهية
        data[memberId] = {
          addedTimestamp: Date.now(),
          removalsRemaining: maxRemovalsPerWeek,
        };
      }

      if (data[memberId].removalsRemaining > 0) {
        // قم بتسجيل البيانات في ملف Data.json
        data[memberId].removalsRemaining--;
        fs.writeFileSync(dataFilePath, JSON.stringify(data, null, 2));

        // أضف السلسلة الزمنية لتتبين كم من الوقت متبقي للعضو لإزالة الرتبة مرة أخرى
        const removalCooldownExpiration = data[memberId].addedTimestamp + removalCooldownPeriod;
        ...
        const embed = {
          color: 0x0099ff,
          title: 'تمت إزالة رتبة',
          description: `تمت إزالة رتبة من ${member.user.username}#${member.user.discriminator}. المرات المتبقية: ${data[memberId].removalsRemaining}`,
        };

        message.channel.send({ embeds: [embed] });

        setTimeout(() => {
          // استعادة إزالة الرتبة بعد انتهاء المدة الزمنية
          member.roles.add(roleIdToRemove);
          message.author.send('لقد استعدت القدرة على إزالة الرتبة مرة أخرى.');
        }, removalCooldownExpiration - Date.now());
      } else {
        const embed = {
          color: 0xff0000,
          title: 'خطأ',
          description: 'لقد استخدمت حدك الأقصى لإزالة الرتب هذا الأسبوع.',
        };
      ...
      const embed = {
        color: 0xff0000,
        title: 'خطأ',
        description: 'يجب أن تكون مشترك بإحدى الرتب المسموح بها لاستخدام هذا الأمر.',
      };
...
// قم بحفظ البيانات عند إيقاف تشغيل البوت
process.on('SIGINT', () => {
  fs.writeFileSync(dataFilePath, JSON.stringify(data, null, 2));
  process.exit();
});
